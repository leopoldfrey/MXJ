...
public class KeyStone extends MaxObject implements JitterNotifiable
{
	...
	public KeyStone(Atom args[])
	{
		...
		declareAttribute("locked", null, "setLocked");
		setTexCtrl(false);
		setEnabled(false);
		update();
	}
			...
			if (eventargs.length >= 2)
			{
				...
				else if (eventname.equalsIgnoreCase("mouse"))
				{
					if(!locked)
					{
						...
						if(button)
						{
							float dtop = (worldcoords[0] - p1.position[0]) / (p2.position[0] - p1.position[0]);
							float dbottom = (worldcoords[0] - p4.position[0]) / (p3.position[0] - p4.position[0]);
							float x = dtop + dbottom - 1;
							float dleft = (worldcoords[1] - p4.position[1]) / (p1.position[1] - p4.position[1]);
							float dright = (worldcoords[1] - p3.position[1]) / (p2.position[1] - p3.position[1]);
							float y = dleft + dright - 1;
							outlet(0, Atom.parse("mouse " + x + " " + y));
						}
					}
				}
			}
