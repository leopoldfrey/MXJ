...
public class JitterGuiTracker implements JitterNotifiable {
	...
	ArrayList<JitterGuiElement> clients = new ArrayList<JitterGuiElement>();
	...
	protected void removeClient(JitterGuiElement client)
	{
...
/*	private void deleteDuplicateClients(JitterGuiElement newclient)
	...
	public void notify(JitterEvent event) 
	{
		...
		{ //otherwise it is a window update, or resize
			...
			float tempcoords[] = new float[] {eventargs[0], eventargs[1], 0.f};
			...
			ArrayList<JitterGuiIntersection> intersectpoints = new ArrayList<JitterGuiIntersection>();
			ArrayList<JitterGuiIntersection> allpoints = new ArrayList<JitterGuiIntersection>();
			int nStilldowns = 0;
			...
			while (it.hasNext())
			{
				...
				i.intersect = false;
				if (e.visible && e.getIntersect(raystart, rayend, i.where))
				{
					i.intersect = true;
			...
			if (intersectpoints.size() > 0)
			{
				...
				frontmost = intersectpoints.get(0).element;
