...
public class pattrwrap extends MaxObject
{
	...
	protected String build = "01/04/2011";
	protected int[] INLET_TYPES = { DataTypes.ALL, DataTypes.ALL, DataTypes.INT, DataTypes.ALL };
	protected int[] OUTLET_TYPES = { DataTypes.ALL, DataTypes.ALL, DataTypes.ALL };
	protected String[] INLET_ASSIST = { "messages to wrapper", "from pattrstorage", "(int) ubumenu item index", "(anything) evaluated ubumenu item" };
	protected String[] OUTLET_ASSIST = { "to pattrstorage" , "to ubumenu" , "bang before storing" };
	
	public pattrwrap(Atom[] atoms)
	{	
...
//		System.err.println("Preset Down is disabled due to a bug in Max 5.0.7");
	...
	public void swap(int i1, int i2)
	{
		if(i1 > 0 && i2 > 0 && i1 < pstname.size() && i2 < pstname.size())
		{
			...
			outlet(OUT_MENU,new Atom[]{Atom.newAtom("setitem"), Atom.newAtom(i1), Atom.newAtom(i1 + " - " + pst2)});
			pstname.set(i1, pst2);
			outlet(OUT_PATTR,new Atom[]{Atom.newAtom("slotname"), Atom.newAtom(i2), Atom.newAtom(pst1)});
			outlet(OUT_MENU,new Atom[]{Atom.newAtom("setitem"), Atom.newAtom(i2), Atom.newAtom(i2 + " - " + pst1)});
